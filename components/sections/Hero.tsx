    ...
    <section ref={ref} className="relative min-h-screen flex items-center justify-center pt-20 px-6 overflow-hidden">
      ...
      <div className="absolute inset-0 w-full h-full z-0">
         ...
         <div className="absolute inset-0">
           <Ballpit 
             key={dimension} // Force remount when dimension changes to update colors
             count={120} 
             gravity={0.25} // Increased gravity by ~60% for more weight/bounce impact
             friction={0.95} // Reduced drag (increased friction coefficient) for longer bounces
             wallBounce={1.2} // Increased wall bounce to add energy on impact
             followCursor={true}
      ...
      <div className="container mx-auto relative z-10 text-center max-w-5xl pointer-events-none">
        <motion.div style={{ y: yText, opacity: opacityText }} className="pointer-events-auto">
            ...
            <motion.div 
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8 text-xs font-bold tracking-widest uppercase border backdrop-blur-md ${dimension === 'growth' ? 'border-gray-200/50 bg-white/30 text-black' : 'border-white/10 bg-white/5 text-propello-green'}`}
            >
            ...
            {/* Subheadline */}
            <motion.p 
                className={`text-lg md:text-xl max-w-2xl mx-auto mb-12 leading-relaxed ${dimension === 'growth' ? 'text-gray-600' : 'text-gray-400'}`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, delay: 0.2 }}
            >
                Propello designs high-performance websites that turn local businesses into category leaders. 
                <span className={`font-serif italic text-xl ${dimension === 'void' ? 'text-propello-green' : 'text-black'}`}> Don't just compete. Dominate.</span>
            </motion.p>
            ...
            >
                <button 
                    className={`group relative px-8 py-4 rounded-full font-bold text-lg overflow-hidden transition-all hover:scale-105 active:scale-95 backdrop-blur-xl border shadow-lg ${
                        dimension === 'growth' 
                ...
                <button 
                    className={`group flex items-center gap-3 px-8 py-4 rounded-full font-medium text-lg transition-all backdrop-blur-xl border shadow-lg ${
                        dimension === 'growth' 
      ...
      >
        <ArrowDownCircle className={`opacity-50 ${dimension === 'growth' ? 'text-black' : 'text-white'}`} size={32} />
      </motion.div>
    </section>
  );
};