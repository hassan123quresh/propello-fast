      ...
      <div className="container mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          <motion.div 
            ...
            className={`p-10 rounded-3xl border ${dimension === 'growth' ? 'bg-red-50/50 border-red-100' : 'bg-red-950/10 border-red-900/30'}`}
          >
            <div className="flex items-center gap-3 mb-8">
                <AlertCircle className="text-red-500" size={32} />
                <h3 className={`text-2xl font-bold font-serif italic ${dimension === 'growth' ? 'text-red-900' : 'text-red-400'}`}>The Reality</h3>
            </div>
            ...
            <ul className="space-y-6">
                {[
                    ...
                    <li key={idx} className="flex items-start gap-4 opacity-70">
                        <span className="block mt-1.5 w-1.5 h-1.5 rounded-full bg-red-500" />
          ...
          >
            ...
            <div className="space-y-8">
                {[
                    ...
                    >
                        <CheckCircle2 className={`flex-shrink-0 transition-colors ${dimension === 'growth' ? 'text-black group-hover:text-propello-green' : 'text-white group-hover:text-propello-green'}`} size={32} />
                        <div>
                            <h4 className="text-xl font-bold mb-2">{item.title}</h4>
                            <p className={`text-lg ${dimension === 'growth' ? 'text-gray-600' : 'text-gray-400'}`}>{item.desc}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
